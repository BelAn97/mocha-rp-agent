...
function MochaRPReporter(runner, options) {
    ...
    let suitPromise = Promise.resolve(null);
    let launchPromise = Promise.resolve(null);
    ...
    let finishLaunch = () => {
        waitForPromise(suitPromise);
        launchPromise = client.finishLaunch(tempLaunchId, {
            status: launchStatus
        }).promise;
    };

    let waitFinishLaunch = () => {
        waitForPromise(launchPromise);
    };
    ...
    let finishSuit = () => {
        waitForPromise(scrPromise);
        suitPromise = client.finishTestItem(getParentId(), {
            status: suitStatus
        }).promise;
        removeParent();
    };
    ...
    runner.on('end', () => {
        try {
            ...
            console.error('Failed to wait finish launch: ', err);
        }
    });
